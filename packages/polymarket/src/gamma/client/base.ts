...
export class GammaBaseClient {
    ...
    this.api = ky.create({
      ...
      hooks: {
        beforeRequest: [
          (request) => {
            if (this.debug) {
              console.log(`[Gamma] ${request.method} ${request.url}`);
            }
          },
        ],
        beforeRetry: [
          async ({ request, retryCount }) => {
            if (this.debug) {
              console.log(
                `[Gamma] Retry attempt ${retryCount} for ${request.url}`,
              );
            }
          },
        ],
        afterResponse: [
          (request, _options, response) => {
            if (this.debug) {
              console.log(
                `[Gamma] Response ${response.status} from ${request.url}`,
              );
            }
            return response;
          },
  ...
  public async request<T>({
    ...
    try {
      ...
      if (this.debug) {
        console.log("[Gamma] Response data:", data);
      }
